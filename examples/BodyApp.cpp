...
float GetDistance(CameraSpacePoint a, CameraSpacePoint b)
{
...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	HWND hWndApp = CreateWindowEx(NULL, L"MyWindowClass", L"Skeleton basics", WS_VISIBLE | WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, NULL, NULL, hInstance, this);
	if (!hWndApp)
	{
...
{
	CBodyBasics* app = NULL;

	if (uMsg == WM_CREATE)
	{
		app = (CBodyBasics*)((LPCREATESTRUCT)lParam)->lpCreateParams;

		SetWindowLongPtr(hWnd, GWLP_USERDATA, (LONG_PTR)app);
	}
	else
	{
		app = (CBodyBasics*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
		if (!app)
			return DefWindowProc(hWnd, uMsg, wParam, lParam);
	}

	return app->DlgProc(hWnd, uMsg, wParam, lParam);
}
				...
				if (pBody)
				{
					...
					if (SUCCEEDED(hr) && bTracked)
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							float distance = GetDistance(joints[JointType_HandLeft].Position, joints[JointType_HandRight].Position);
							unsigned int frequency = (unsigned int)(distance * 1000);
							audioDevice->SetWaveParameters(frequency, 0.5);
